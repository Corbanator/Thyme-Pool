...
function newFrame(frame, dTime, ballSet, forces, frameWidth, frameHeight) {
...
function simulate(dTime, simTime, ballSet, frame1 , forces, frameWidth, frameHeight) {
    var simulation = {
        ballSet: ballSet,
        frames: new Array
    }
    ...
    for(var i = 0; i < simTime; i+= dTime){
        var nFrame = newFrame(frame, dTime, ballSet.pos, getForces(frameCount,forces), frameWidth, frameHeight);
        simulation.frames.push(frame);
        frame = nFrame;
        frameCount++;
    }
    simulation.frames[0] = frame1;
    simulation.frameTime = dTime;
    simulation.simTime = simTime;
    return simulation;
}

function getForces(frameNum, forces){
    let returnForces = [];
    forces.forEach(element => {
     if(element.frame == frameNum){
        returnForces.push(element);
     }   
